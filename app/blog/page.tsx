...
const Blog = () => {
  ...
  const {res:  blogs = [], isLoading, error } = useSelector(
  ...
  const {
    ...
    error: errorProfile,
  } = useSelector((state) => state.getAutherData || {});
  ...
  // Ensure blogs are available before filtering to avoid null errors
  const filteredBlogs =
    ...
    for (let i = 1; i <= Math.min(visiblePages, totalPages); i++) {
      paginationButtons.push(
        <button
          ...
          className={`mx-1 flex h-9 min-w-[36px] items-center justify-center rounded-md px-4 text-sm ${
            currentPage === i
